      ...
      return (
        <div>
          <div style={{ margin: '10px' }}>
            <Breadcrumb>
              <Breadcrumb.Item><Link to="/">Sites</Link></Breadcrumb.Item>
              <Breadcrumb.Item>Dance marathon</Breadcrumb.Item>
            </Breadcrumb>
          </div>
          <div style={{ margin: '10px' }}>
            <Button style={{ marginBottom: 10 }} onClick={() => ipcRenderer.send('file-dialog')}>
      ...
      <div>
        ...
        <Tabs size="large" defaultActiveKey="1" style={{ width: '100vw' }}>
          <TabPane tab="Files" key="1" style={{ marginBottom: 10 }}>
            <Button style={{ marginBottom: 10, marginLeft: 10 }} onClick={() => {
              glob(`${this.state.selectedFolder}/**/*.*`, (err, files) => {
                const formattedFiles = files.map(file => file.replace(this.state.selectedFolder, '')).filter(file => file !== '');
                this.setState({ files: formattedFiles });
              });
            }}><Icon type="reload" />Refresh files</Button>
            <Button style={{ marginBottom: 10, marginLeft: 10 }} onClick={() => {
              const hashedFiles = hashFiles(this.state.selectedFolder, this.state.files);
                ...
                .then(res => {
                  ...
                  for (const [key, value] of Object.entries(hashedFiles)) {
                    if (requiredForUpload.includes(value)) {
                      fs.readFile(`${this.state.selectedFolder}${key}`, 'utf8', function (err, data) {
                        if (err) {
            ...
            <Button disabled style={{ marginLeft: 10 }}><Icon type="download" />Download Files</Button>
            <List
              style={{ height: '100%', minHeight: '350px' }}
              bordered
              dataSource={this.state.files}
              renderItem={item => (
                <List.Item actions={[<a><Icon title="Upload this file" type="upload" /></a>, false ? <Spin /> : <Icon  title="Pending" style={{color: 'orange'}} type="exclamation" />]}>
                  <List.Item.Meta title={item} />
                </List.Item>)}
            />
          </TabPane>
          <TabPane tab="Form" key="2">Content of Tab Pane 1</TabPane>
          <TabPane tab="Submissions" key="3">Content of Tab Pane 1</TabPane>
          <TabPane tab="Hooks" key="4">Content of Tab Pane 1</TabPane>
          <TabPane tab="Snippets" key="5">Content of Tab Pane 1</TabPane>
          <TabPane tab="Metadata" key="6">Content of Tab Pane 1</TabPane>
          <TabPane tab="Deploys" key="7">Content of Tab Pane 1</TabPane>
          <TabPane tab="Account" key="8">Content of Tab Pane 1</TabPane>
        </Tabs>
      </div>
    );
  }
